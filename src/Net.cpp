...
#import "..\Turuga_Match_v1.0.tlb" no_namespace named_guids
//SV - End
...
//SV - Begin
///Turunga IP Search
void CTinyCadView::OnSpecialTurungaIPSearch(){
	
   CDlgAboutTurunga().DoModal();
  
}

void CTinyCadView::OnSpecialTurungaFetchSimilarIP(){
	IManagedInterface *cpi = NULL;
	 // Initialize COM and create an instance of the InterfaceImplementation class:
   // Initialize COM and create an instance of the InterfaceImplementation class:
   CoInitialize(NULL);
   //first argument = CLSID_<classname>
   //fourth arg = IID_<interface name>
   //class and interface needs to have GUID
   HRESULT hr = CoCreateInstance(CLSID_TMatch,
   ...
   if (FAILED(hr))
   {
	...
	//printf("Percentage match is %g\n",cpi->matchPercent(@"c:\Workspace\netlist.txt", @"c:\Workspace\netlist2.txt"));
	 /*  if(cpi->matchPercent("c:\\Workspace\\netlist.txt", "c:\\Workspace\\netlist2.txt") == 21.3)
	   {
		   CDlgAboutTurunga().DoModal();
		   printf("Percentage read = %g\n",cpi->matchPercent("c:\\Workspace\\netlist.txt", "c:\\Workspace\\netlist2.txt"));

	   }*/
	   ///static_cast<CTinyCadMultiDoc*>(GetDocument()))->GetPathName() returns cstring format
	   ///to convert into bstr use ->AllocSysString()

	   cpi->sendFromTinycad(static_cast<CTinyCadMultiDoc*>(GetDocument())->GetPathName().AllocSysString(), "..\\test");
